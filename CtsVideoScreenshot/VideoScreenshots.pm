...
### $per_min_frames
	...
	if ( $drama =~ /E/ ){
		...
		CutOutImage_screenshot_img( $in_video, 176, 126, $out_img_path, $per_min_frames);
...
### $xml_path
	...
	if ( $in_video =~ /E/ ){
		...
		ffmpeg_screenshot_img( $in_video, 176, 126, $duration_sec, $out_img_path);
...
### $out_xml_path
	...
	if ( $in_video =~ /E/ ){
		...
		mplayer_screenshot_img( $in_video, 176, 126, $duration_sec, $out_img_path);
...
sub mplayer_screenshot_img{
	...
	my ($leftEdge, $rightEdge, $topEdge, $bottomEdge) = detectionImgBlackborder( $new_pic ) if ( -f $new_pic);
	$preWidth = $width + $leftEdge + $rightEdge + 4;
	$preHeight = $height + $topEdge + $bottomEdge + 4;
	...
	if ($leftEdge + $rightEdge + $topEdge + $bottomEdge == 0 ){
		...
		$scale_param = qq(scale=$preWidth:$preHeight,crop=$width:$height);
	}
	for( $time_offset=$timeInterval, $ii=1; $time_offset <= $time_len; $time_offset += $timeInterval, $ii++ ){
		print "#### ii=$ii, time_offset=$time_offset \n";
...
sub documentary_iphone_screenshot_img{
	...
	if ($leftEdge + $rightEdge + $topEdge + $bottomEdge == 0 ){
		...
		$scale_param = qq(scale=$preWidth:-3,crop=$width:$height);
	}
    my ($time_offset, $ii);
	for( $time_offset = $timeInterval, $ii=1; $time_offset <= $time_len; $time_offset += $timeInterval, $ii++ ){
		print "#### ii=$ii, time_offset=$time_offset \n";
...
sub ffmpeg_run_screenshot {
	...
	my $cmd = qq(ffmpeg -i $file_name -y -f image2 -ss $off_time -vf "$scale_param" -vframes 1 $new_pic);
	`$cmd`;
	return ($new_pic, $cmd);
}
