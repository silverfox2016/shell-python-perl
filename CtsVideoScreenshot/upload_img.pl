...
sub chown_mod{
	...
	my ($path) = $remote_path =~ /(\/(\w+\/)+(\d+))/;
	`ssh 58.68.228.46 "chown -R root:content $path && chmod -R 775 $path"`;
	return;
	}
